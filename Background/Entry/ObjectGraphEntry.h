...
namespace ObjectGraphEntry
{
	...
	inline void GenerateUperTree() 
	{
		...
		StorageMgr.ObjectGraphUperProgressBarTotalValue.Set(ObjectGraphState.GetMostUperObjectVectorEvent.Cnt * ObjectGraphConf.KeepUperNodeNumber); // std::pow(KeepUperNodeNumber, ObjectGraphConf.TreeLevel)
	...
	inline void Entry()
	{
		if (ObjectGraphState.GetMostUperObjectVectorEvent.State == ProcessState::Start) {
			ObjectGraphState.GetMostUperObjectVectorEvent.State = ProcessState::Processing;
			EventHandler::NotifyEvent(NotificationConfig::NotiyType::Info, "[Execute] Get Most Uper Object Vector");
			GetMostUperObjectVector(ObjectGraphState.GetMostUperObjectVectorEvent.Cnt);
			GenerateUperTree();
			ObjectGraphState.GetMostUperObjectVectorEvent.State = ProcessState::Completed;
		}
	}
}

void ObjectGraphEntry::GetMostUperObjectVector(int keepCnt) {
	StorageMgr.ObjectGraphUperProgressBarValue.Set(0);
