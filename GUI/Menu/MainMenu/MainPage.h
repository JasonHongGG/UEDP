...
namespace MainPage
{
	void Render(MainMenuCurPage PageID) {
		if (PageID != MainMenuCurPage::Main) return;
		...
		ImGui::BeginChild("Option", ImVec2(0, 0), true);
		{
			...
			ImGui::DummySpace();
			{
				...
				MyComboList("##Window Selector", SelectProcessWindowConf.WindowTitleSelectIndex, SelectProcessWindowConf.WindowTitleList).Show();
				ImGui::PopItemWidth();
				ImGui::PopFont();
