...
UpdateMarkerResult UpdateMarker(
	...
	int& markerCount,
	ImGradientHDRState& temporaryState,
...
bool ImGradientHDR(int32_t gradientID, ImGradientHDRState& state, ImVec2 size, bool isMarkerShown)
{
	bool changed = false;
	...
	const auto barHeight = size.y;
	const auto markerWidth = 10;
	const auto markerHeight = 15;

	if (isMarkerShown)
	{
		const auto resultAlpha = UpdateMarker(state.Alphas, state.AlphaCount, state, ImGradientHDRMarkerType::Alpha, "a", originPos, width, markerWidth, markerHeight, MarkerDirection::ToLower);

		changed |= resultAlpha.isChanged;

		if (state.draggingMarkerType == ImGradientHDRMarkerType::Alpha)
		{
			SortMarkers(state.Alphas, state.AlphaCount, state.selectedIndex, state.draggingIndex);
		}
	...
	for (int y = 0; y * gridSize < barHeight; y += 1)
	{
		for (int x = 0; x * gridSize < width; x += 1)
		{
			...
			int hgrid = std::min(gridSize, static_cast<int>(barHeight) - y * gridSize);
	...
	if (isMarkerShown)
	{
		...
		const auto resultColor = UpdateMarker(state.Colors, state.ColorCount, state, ImGradientHDRMarkerType::Color, "c", originPos, width, markerWidth, markerHeight, MarkerDirection::ToUpper);

		changed |= resultColor.isChanged;

		if (state.draggingMarkerType == ImGradientHDRMarkerType::Color)
		{
			SortMarkers(state.Colors, state.ColorCount, state.selectedIndex, state.draggingIndex);
		}
